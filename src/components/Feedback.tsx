...
function CheckIcon(props: React.ComponentPropsWithoutRef<'svg'>) {
  return (
    <svg viewBox='0 0 20 20' aria-hidden='true' {...props}>
      ...
      <path fill='none' strokeLinecap='round' strokeLinejoin='round' strokeWidth='1.5' d='m6.75 10.813 2.438 2.437c1.218-4.469 4.062-6.5 4.062-6.5' />
    </svg>
  );
}
...
const FeedbackForm = forwardRef<React.ElementRef<'form'>, Pick<React.ComponentPropsWithoutRef<'form'>, 'onSubmit'>>(function FeedbackForm(
  { onSubmit },
  ref,
) {
  return (
    <form ref={ref} onSubmit={onSubmit} className='absolute inset-0 flex items-center justify-center gap-6 md:justify-start'>
...
export function Feedback() {
  ...
  return (
    <div className='relative h-8'>
      <Transition show={!submitted} as={Fragment} leaveFrom='opacity-100' leaveTo='opacity-0' leave='pointer-events-none duration-300'>
        <FeedbackForm onSubmit={onSubmit} />
      </Transition>
      <Transition show={submitted} as={Fragment} enterFrom='opacity-0' enterTo='opacity-100' enter='delay-150 duration-300'>
        <FeedbackThanks />
      </Transition>
    </div>
  );
