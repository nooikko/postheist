...
import { type AutocompleteApi, createAutocomplete, type AutocompleteState, type AutocompleteCollection } from '@algolia/autocomplete-core';
...
function HighlightQuery({ text, query }: { text: string; query: string }) {
  return (
    <Highlighter highlightClassName='underline bg-transparent text-emerald-500' searchWords={[query]} autoEscape={true} textToHighlight={text} />
  );
}
...
function SearchResult({
  ...
  const sectionTitle = navigation.find((section) => section.links.find((link) => link.href === result.url.split('#')[0]))?.title;
  ...
  return (
    ...
    >
      <div id={`${id}-title`} aria-hidden='true' className='text-sm font-medium text-zinc-900 group-aria-selected:text-emerald-500 dark:text-white'>
        <HighlightQuery text={result.title} query={query} />
      </div>
      {hierarchy.length > 0 && (
        <div id={`${id}-hierarchy`} aria-hidden='true' className='mt-1 truncate whitespace-nowrap text-2xs text-zinc-500'>
    ...
    return (
      <div className='p-6 text-center'>
        ...
        <p className='mt-2 text-xs text-zinc-700 dark:text-zinc-400'>
          Nothing found for <strong className='break-words font-semibold text-zinc-900 dark:text-white'>&lsquo;{query}&rsquo;</strong>. Please try
          again.
        </p>
      </div>
    );
  ...
  return (
    <ul {...autocomplete.getListProps()}>
      {collection.items.map((result, resultIndex) => (
        <SearchResult key={result.url} result={result} resultIndex={resultIndex} autocomplete={autocomplete} collection={collection} query={query} />
      ))}
    </ul>
  );
}
...
function SearchDialog({ open, setOpen, className }: { open: boolean; setOpen: (open: boolean) => void; className?: string }) {
