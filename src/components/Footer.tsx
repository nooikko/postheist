...
function PageLink({ label, page, previous = false }: { label: string; page: { href: string; title: string }; previous?: boolean }) {
  return (
    <>
      <Button href={page.href} aria-label={`${label}: ${page.title}`} variant='secondary' arrow={previous ? 'left' : 'right'}>
        {label}
      </Button>
      <Link
        href={page.href}
...
function SmallPrint() {
  return (
    <div className='flex flex-col items-center justify-between gap-5 border-t border-zinc-900/5 pt-8 sm:flex-row dark:border-white/5'>
      <p className='text-xs text-zinc-600 dark:text-zinc-400'>&copy; Copyright {new Date().getFullYear()}. All rights reserved.</p>
